...
http_parser_settings http_header_parser_settings = {
...
void http_header_init(struct http_header_t *http_header)
{
    INIT_LIST_HEAD(&http_header->list);
}

void http_header_add(struct http_header_t *http_header, const char *field, const char *value)
{
    //struct http_header_entry_t *entry;
}

void http_header_del(struct http_header_t *http_header, const char *field)
{
    struct http_header_entry_t *entry;
    struct http_header_entry_t *entry_tmp;
    list_for_each_entry_safe(entry, entry_tmp, &http_header->list, node) {
...
const char* http_header_find(struct http_header_t *http_header, const char *field)
{
    struct http_header_entry_t *entry;
    list_for_each_entry(entry, &http_header->list, node) {
        if (strcasecmp(entry->field, field) == 0) {
            return entry->value;
        }
    }
...
void http_header_sprintf(struct http_header_t *http_header, enum http_parser_type type, struct string_t *string)
{
    struct http_header_entry_t *entry;

    if (type == HTTP_REQUEST) {
        string_sprintf(string, "%s %s HTTP/%d.%d\r\n", http_method_str(http_header->method), http_header->url, http_header->http_major, http_header->http_minor);
    } else if (type == HTTP_RESPONSE) {
        string_sprintf(string, "HTTP/%d.%d %d %s\r\n", http_header->http_major, http_header->http_minor, http_header->status_code, http_status_str(http_header->status_code));
    }
    list_for_each_entry(entry, &http_header->list, node) {
...
void http_header_uninit(struct http_header_t *http_header)
{
    struct http_header_entry_t *entry;
    while (!list_empty(&http_header->list)) {
        entry = d_list_head(&http_header->list, struct http_header_entry_t, node);
        list_del(&entry->node);
        mem_free(entry->field);
        mem_free(entry->value);
        mem_free(entry);
...
static int on_url(http_parser *http_parser, const char *buf, size_t len)
{
    struct http_header_t *http_header = http_parser->data;
    size_t old_len = 0;

    if (http_header->url) {
        old_len = strlen(http_header->url);
        http_header->url = mem_realloc(http_header->url, old_len + len + 1);
    }
    memcpy(http_header->url + old_len, buf, len);
    http_header->url[old_len + len] = '\0';
    return 0;
}
...
static int on_header_field(http_parser *http_parser, const char *buf, size_t len)
{
    ...
    if (list_empty(&http_header->list)) {
        entry = mem_malloc(sizeof(struct http_header_entry_t));
        entry->field = NULL;
        entry->value = NULL;
        list_add_tail(&entry->node, &http_header->list);
    } else {
        entry = d_list_tail(&http_header->list, struct http_header_entry_t, node);
        if (entry->value) {
            entry = mem_malloc(sizeof(struct http_header_entry_t));
            entry->field = NULL;
            entry->value = NULL;
...
static int on_headers_complete(http_parser *http_parser)
{
    ...
    if (http_parser->type == HTTP_REQUEST) {
        http_header->method = http_parser->method;
    } else if (http_parser->type == HTTP_RESPONSE) {
        http_header->status_code = http_parser->status_code;
    }
    http_header->http_major = http_parser->http_major;
    http_header->http_minor = http_parser->http_minor;
    return 1;
}
