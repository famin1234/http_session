...
static const uint8_t * md5_body(struct md5_t *ctx, const uint8_t *data, size_t size)
{
  uint32_t       a, b, c, d;
...
void md5_update(struct md5_t *ctx, const void *data, size_t size)
{
  ...
  if (used) {
    ...
    memcpy(&ctx->buffer[used], data, nfree);
    data = (uint8_t *) data + nfree;
    size -= nfree;
...
void md5_final(uint8_t result[16], struct md5_t *ctx)
{
  ...
  if (nfree < 8) {
    memset(&ctx->buffer[used], 0, nfree);
    (void) md5_body(ctx, ctx->buffer, 64);
    used = 0;
    nfree = 64;
  }
  ...
  memset(&ctx->buffer[used], 0, nfree - 8);
