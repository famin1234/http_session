...
int epoll_module_init(struct net_module_t *net_module)
{
    ...
    net_module->sub_module = (void *)epoll_module;
    return 0;
}
...
int epoll_module_mod(struct net_module_t *net_module, struct conn_t *conn, int events)
{
    ...
    epoll_module = (struct epoll_module_t *)net_module->sub_module;
    if (conn->events == 0) {
...
{
    struct epoll_module_t *epoll_module = (struct epoll_module_t *)net_module->sub_module;
    ...
    for (i = 0; i < nfds; i++) {
        ...
        if (evs[i].events & (EPOLLIN | EPOLLERR | EPOLLHUP)) {
            ...
            if (!conn->flags.active) {
                list_add_tail(&conn->active_node, &net_module->active_list);
                conn->flags.active = 1;
            }
        }
